...
static void posix_group_task_destructor(Slapi_Task *task);
static void posix_group_fixup_task_thread(void *arg);
static int posix_group_fix_memberuid_callback(Slapi_Entry *e, void *callback_data);
                    ...
                    void *arg)
{
	...
	*returncode = LDAP_SUCCESS;
	...
	thread = PR_CreateThread(PR_USER_THREAD, posix_group_fixup_task_thread,
...
static int posix_group_del_memberuid_callback(Slapi_Entry *e, void *callback_data)
{
	int rc = 0;
	LDAPMod mod;
	LDAPMod *mods[2];
...
static int posix_group_fix_memberuid(char *dn, char *filter_str, void *txn)
{
	int rc = 0;
	...
	rc = slapi_search_internal_callback_pb(search_pb,
		&callback_data,
		0, posix_group_fix_memberuid_callback,
		0);
...
static int posix_group_fix_memberuid_callback(Slapi_Entry *e, void *callback_data)
{
    ...
    if (rc == 0) {                 /* Found uniquemember, so...  */
        ...
        slapi_log_error( SLAPI_LOG_PLUGIN, POSIX_WINSYNC_PLUGIN_NAME,"_fix_memberuid scan uniquemember, group %s\n",dn);
        for (
            i = slapi_attr_first_value(obj_attr, &uniqval);
            i != -1;
